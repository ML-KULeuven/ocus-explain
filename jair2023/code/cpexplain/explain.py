...
def explain_interleaved_ous(soft, soft_weights=None,  hard=[], solver="ortools", verbose=0):
    curr_sol = set()
    ...
    while(curr_sol != full_sol):
        ...
        ocus_expl = explain_one_step_interleaved(hard + reified_soft, all_lit, cost, remaining_to_explain, solver, verbose)
...
def explain_one_step_interleaved(hard, soft_lit, cost, remaining_sol_to_explain, solver="ortools", verbose=False):
    """
        Interleaving Optimal Unsatisfiable Subsets (Inter. OUS) for CSP explanations [1]
        ...
        Artificial intelligence, 32(1), 57-95.
    """
    ## ----- OPTIMISATION MODEL for every literal to explain------
    ## -------------- VARIABLES -----------------
    hs_vars, hittingset_solver = dict(), dict()

    # mapping between hitting set variables hs_var <-> Iend
    varmap_hs_sat, varmap_sat_hs = dict(), dict()

    for lit in enumerate(remaining_sol_to_explain):
        F_l = list(soft_lit) + [~lit]
        hs_vars[lit] = boolvar(shape=len(soft_lit) + 1)
        varmap_hs_sat[lit] = dict( (hs_vars[lit][id], var) for id, var in enumerate(F_l))
        varmap_sat_hs[lit] = dict( (var, hs_vars[lit][id]) for id, var in enumerate(F_l))

        ## ----------------- MODEL ------------------
        hs_mip_model = Model(
            # exactly one variable to explain and it must be at least the negated literal!
            hs_vars[lit][len(F_l)-1] == 1,
            # Objective: min sum(x_l * w_l) over all l in I + (-Iend \ -I)
            minimize=sum(hs_var * cost(varmap_hs_sat[lit][hs_var]) for hs_var in hs_vars[lit]) 
        )

        # instantiate hitting set solver
        hittingset_solver[lit] = SolverLookup.lookup(solver)(hs_mip_model)

    ## ----- SAT solver model ----
    SAT = SolverLookup.lookup(solver)(Model(hard))

    #####  keeping a priority queue for the literals to explain
    pq = []
    for lit in remaining_sol_to_explain:
        hittingset_solver[lit].solve()

        # Get hitting set
        HS = hs_vars[lit][hs_vars[lit].value() == 1]

        # map to vars of formula F
        S = set(varmap_hs_sat[lit][hs_var] for hs_var in HS)
        hq.heappush(pq, (sum(cost(l) for l in S), lit, HS, S))

    while(True):
        (_, lit, HS, S) = hq.heappop(pq)

        # not incremental => don't care about the literals not yet derived!
        F_l = set(soft_lit) | set([~lit])
        ...
        # satisfying model
        S = set(v if v.value() else ~v for v in F_l)

        # compute complement of model in formula F
        C =  F_l - S

        set_to_hit = set(varmap_sat_hs[lit][var] for var in C)

        # Add complement as a new set to hit: sum x[j] * hij >= 1
        hittingset_solver[lit] += (sum(set_to_hit) >= 1)
