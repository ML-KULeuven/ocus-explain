...
class HitmanHS(object):

    def __init__(self, f, F, A):
        self.opt_model = Hitman()

    def addCorrectionSet(self, f, C: set):
        ...
        self.opt_model.hit(to_hit=list(C), weights=[f(l) for l in C])
...
class OptHS(object):
    ...
    def addCorrectionSets(self, H: list):
        """Add new constraint of the form to the optimization model,
        mapped back to decision variable lit => x[i].
    ...
    def OptHittingSet(self, end_time_timeout=None):
        """Compute conditional Optimal hitting set.
        ...
        """
        if end_time_timeout:
            self.opt_model.Params.TimeLimit = end_time_timeout - time.time()
...
class CondOptHS(object):
    def __init__(self, Iend: set, I: set):
        """
        # Optimisation model:
    ...
    def CondOptHittingSet(self, end_time_timeout=None):
        """Compute conditional Optimal hitting set.
...
class UNSATCondOptHS(CondOptHS):
    def __init__(self, Iend: set, I: set):
        """
        # Optimisation model:
        ...
        """
        Iexpl = set(-l for l in Iend - I)

        self.allLits = list(I) + list(Iexpl)
        self.nAllLits = len(self.allLits)
        ...
        # CONSTRAINTS
        # every explanation contains 1 neg Lit.
        posnegIexpl = range(len(I), self.nAllLits)
