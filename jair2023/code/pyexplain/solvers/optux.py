...
import getopt
import os
from pyexplain.solvers.hittingSet import OptHS, OptUxHS
...
class OptUx(object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, exhaust=False,
            minz=False, trim=False, disable_dijsoint=False,verbose=0):
        """
            Constructor.
        """
        ...
        self.preprocess_time = time.time() - tpre
        ...
        self.n_to_hit = len(to_hit)
        self.n_to_units = len(self.units)
        ...
        if disable_dijsoint:
            self.hitman = Hitman(solver=solver, htype='sorted', mxs_adapt=adapt,
    ...
    def compute(self):
        ...
        while True:
            ...
            if res == False:
                ...
                self.compute_time =  time.time() - tstart
...
class OptUxMIP(object):
    def __init__(self, formula, solver='g3', adapt=False, exhaust=False,
        ...
        self.to_hit, self.units = [], []

        # enumerating disjoint MCSes (including unit-size MCSes)
        if not disable_dijsoint:
            # creating an unweighted copy
            unweighted = formula.copy()
            unweighted.wght = [1 for w in unweighted.wght]
            # applying disjoint mcs-enumeration
            self.to_hit, self.units = self._disjoint(unweighted, solver, adapt, exhaust, minz, trim)

            # print(self.to_hit, self.units)

        if self.verbose > 2:
            print('c mcses: {0} unit, {1} disj'.format(len(self.units),
                len(self.to_hit) + len(self.units)))

        self.hitman = OptUxHS(self.sels, self.weights)
    ...
    def compute(self):
        ...
        while True:
            ...
            if res == False:
                ...
                # self.hitman.block(hs)
                # self.cost = self.hitman.oracle.cost + len(self.units)
                return sorted(map(lambda s: self.smap[s], self.units + list(hs)))
            else:
                # the candidate subset is satisfiable,
                # thus extracting a correction subset
                model = self.oracle.get_model()
                ...
                # hitting the new correction subset
                self.hitman.addCorrectionSet(cs)
