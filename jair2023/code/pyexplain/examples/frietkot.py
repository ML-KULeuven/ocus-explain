...
def hardBarronProblem():
    """
    Logic Puzzle taken from 
    https://logic.puzzlebaron.com/play.php?u2=5ea6cdc012571d8b053ace3e097d65ad

    """
    tortoises = ["Chewie", "Methuselah", "Shelly", "Snappy", "Speedy", "Toredo", "Yoda"]
    species = ["armored", "black_neck", "horned", "pitch_belly", "ringed", "swoop_backed", "two_rimmed"]
    countries = ["Algeria", "Guinea", "Jamaica", "Oman", "Uruguay", "Yemen", "Zimbabwe"]
    ages = ["14","32", "50", "68", "86", "104", "122"]
    
    ages_countries = Relation(ages, countries)
    ages_species = Relation(ages, species)
    ages_tortoises = Relation(ages, tortoises)
    countries_species = Relation(countries, species)
    countries_tortoises = Relation(countries, tortoises)
    species_tortoises = Relation(species, tortoises)

    rels = [ages_tortoises ,ages_species ,ages_countries ,species_tortoises ,countries_species ,species_turtoises]
    ...
    for x in ages:
        for y in countries:
            for z in species:
                t0 = to_cnf(implies(ages_species[x, z] & countries_species[y, z], ages_countries[x, y]))
                [trans.append(implies(bv_trans[0], clause)) for clause in t0]

                t1 = to_cnf(implies(~ages_species[x, z] & countries_species[y, z], ~ages_countries[x, y]))
                [trans.append(implies(bv_trans[1], clause)) for clause in t1]

                t2 = to_cnf(implies(ages_species[x, z] & ~countries_species[y, z], ~ages_countries[x, y]))
                [trans.append(implies(bv_trans[2], clause)) for clause in t2]

    for x in ages:
        for y in countries:
            for z in tortoises:
                t3 = to_cnf(implies(ages_turtoises[x, z] & countries_tortoises[y, z], ages_countries[x, y]))
                [trans.append(implies(bv_trans[3], clause)) for clause in t3]

                t4 = to_cnf(implies(~ages_turtoises[x, z] & countries_tortoises[y, z], ~ages_countries[x, y]))
                [trans.append(implies(bv_trans[4], clause)) for clause in t4]

                t5 = to_cnf(implies(ages_turtoises[x, z] & ~countries_tortoises[y, z], ~ages_countries[x, y]))
                [trans.append(implies(bv_trans[5], clause)) for clause in t5]

    for x in ages:
        for y in species:
            for z in tortoises:
                t6 = to_cnf(implies(ages_turtoises[x, z] & species_tortoises[y, z], ages_species[x, y]))
                [trans.append(implies(bv_trans[6], clause)) for clause in t6]

                t7 = to_cnf(implies(~ages_turtoises[x, z] & species_tortoises[y, z], ~ages_species[x, y]))
                [trans.append(implies(bv_trans[7], clause)) for clause in t7]

                t8 = to_cnf(implies(ages_turtoises[x, z] & ~species_tortoises[y, z], ~ages_species[x, y]))
                [trans.append(implies(bv_trans[8], clause)) for clause in t8]
    ...
    for x in countries:
        for y in species:
            for z in tortoises:
                t9 = to_cnf(implies(countries_tortoises[x, z] & species_tortoises[y, z], countries_species[x, y]))
                [trans.append(implies(bv_trans[9], clause)) for clause in t9]

                t10 = to_cnf(implies(~countries_tortoises[x, z] & species_tortoises[y, z], ~countries_species[x, y]))
                [trans.append(implies(bv_trans[10], clause)) for clause in t10]

                t11 = to_cnf(implies(countries_tortoises[x, z] & ~species_tortoises[y, z], ~countries_species[x, y]))
                [trans.append(implies(bv_trans[11], clause)) for clause in t11]

    clues = []
    bv_clues = [BoolVar() for i in range(13)]
    ...
    clue0 = []
    for age_rimmed_tortoise in ages:
        for age_armored_tortoise in age:
            if age_rimmed_tortoise != age_armored_tortoise and int(age_rimmed_tortoise)== int(age_armored_tortoise) + 18:
                clue0.append(implies(ages_species[age_rimmed_tortoise, "two_rimmed"], ages_species[age_armored_tortoise, "armored"]))

    [clues.append(implies(bv_clues[0], cl)) for cl in clue0 ]

    # "The pitch belly tortoise is 18 years older than the one from Zimbabwe.",
    clue1 = []
    [clues.append(implies(bv_clues[1], cl)) for cl in clue1_cnf ]

    # "The swoop-backed tortoise is younger than the horned tortoise.",
    clue2 = []
    [clues.append(implies(bv_clues[2], cl)) for cl in clue2_cnf ]

    # "Speedy is from Algeria.",
    clues.append(implies(bv_clues[3], countries_tortoises["Algeria", "Speedy"]))

    # "The two rimmed tortoise is either Chewie or Toredo.",
    clue4 = []
    [clues.append(implies(bv_clues[4], cl)) for cl in clue4_cnf]

    # "The one from Zimbabwe isn't the ringed tortoise.",
    clue5 = []
    [clues.append(implies(bv_clues[5], cl)) for cl in clue5_cnf]

    # "The tortoise from Oman is 90 years older than the one from Uruguay.",
    clue6 = []
    [clues.append(implies(bv_clues[6], cl)) for cl in clue6_cnf]

    # "The seven animals are Speedy, the two rimmed tortoise, the swoop-backed tortoise, the animal from Oman, the 86-year-old animal, the animal from Zimbabwe and the 14-year-old tortoise.",
    clue7 = []
    [clues.append(implies(bv_clues[7], cl)) for cl in clue7_cnf]

    # "Methuselah is older than Shelly.",
    clue8 = []
    for younger in ages:
        for older in ages:
            if int(younger) < int(older):
                clues8.append(ages_tortoises[younger, "Shelly"] & ages_tortoises[older, "Methuselah"])
    clue8_cnf = to_cnf(any(clues8))
    [clues.append(implies(bv_clues[8], cl)) for cl in clue8_cnf]

    # "The tortoise from Uruguay is either the pitch belly tortoise or the 14-year-old animal.",
    clue9 = []
    [clues.append(implies(bv_clues[9], cl)) for cl in clue9_cnf]

    # "Chewie is from Zimbabwe.",
    clues.append(implies(bv_clues[10], countries_tortoises["Zimbabwe", "Chewie"]))

    # "Toredo is younger than the one from Jamaica.",
    clue11 = []
    [clues.append(implies(bv_clues[11], cl)) for cl in clue11_cnf]

    # "Of the black neck tortoise and the animal from Yemen, one is named Snappy and the other is named Shelly."
    clue12 = []
    [clues.append(implies(bv_clues[12], cl)) for cl in clue12_cnf]
    ...
    rel_strs = [TODO_FILL_IN_THE_RELATIONS_STR]
    ...
    explainable_facts = set()
    bvRels = {}
    for rel, relStr in zip(rels, rel_strs):
        rowNames = list(rel.df.index)
        columnNames = list(rel.df.columns)
        for r in rowNames:
            for c in columnNames:
...
def simpleProblem():
    (mayo, andalouse, ketchup) = BoolVar(3)

    c0 = ~mayo | ~andalouse | ketchup
    c1 = ~mayo | andalouse | ketchup
    c2 = mayo
    c3 = ~andalouse | ~ketchup
    ...
    matching_table = {
        (mayo.name + 1): "mayo", (andalouse.name + 1): "andalouse", ketchup.name + 1: "ketchup",
        -(mayo.name + 1): "~mayo", -(andalouse.name + 1): "~andalouse", -(ketchup.name + 1): "~ketchup",
    }
    matching_table.update({l: f"c{id}" for id, l in enumerate(assumptions)})
    # weights.update({})

    return f_cnf_ass, [[l] for l in assumptions], weights, f_user_vars, matching_table
